...
vector<string> move_x(int N, vector<string> &C) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (fuku_x.size() == 0) {
            ...
            // 全部右の場合を初期値にする
            int left_index = -1;
            int left_count = N - oni_x[0];
            for (int k = 0; k < oni_x.size(); k++) {
                // oni_x[k] まで左に動かす
                int count = oni_x[k] + 1;
                if (k != oni_x.size() - 1) {
                    count *= 2;
            ...
            // 1つ以上左の場合
            for (int j = 0; j < oni_x[left_index] + 1; j++) {
                move_log.push_back("L " + to_string(i));
            }
            for (int j = 0; j < left_count - oni_x[left_index] - 1; j++) {
                move_log.push_back("R " + to_string(i));
            }
            C[i] = string(N, '.');
            continue;
        ...
        if (oni_left_fuku != -1) {
            ...
            // 福のいる列で、福より先に鬼が出現するか確認
            bool exist_oni = false;
            for (int k = 0; k < fuku_x.size(); k++) {
                bool meet_fuku = false;
                for (int ii = i - 1; ii >= 0; ii--) {
                    if (C[ii][fuku_x[k] - oni_left_fuku - 1] == 'o') {
                        meet_fuku = true;
                        break;
                    }
                    if (!meet_fuku &&
                        C[ii][fuku_x[k] - oni_left_fuku - 1] == 'x') {
                        exist_oni = true;
                        break;
                    }
                }
        ...
        if (oni_right_fuku != -1) {
            ...
            for (int k = 0; k < fuku_x.size(); k++) {
                bool meet_fuku = false;
                for (int ii = i - 1; ii >= 0; ii--) {
                    if (C[ii][fuku_x[k] + (N - oni_right_fuku)] == 'o') {
                        meet_fuku = true;
                        break;
                    }
                    if (!meet_fuku &&
                        C[ii][fuku_x[k] + (N - oni_right_fuku)] == 'x') {
                        exist_oni = true;
                        break;
                    }
                }
